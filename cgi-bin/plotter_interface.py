...
def graph(name, functions, settings): # x_pos, y_pos, scale):
    ...
    task = grapher.main(name, functions, x_pos, y_pos, scale / 10 * const.PIXELS_PER_MM)
    print_task(name, task)

def bezier(name, p_list, settings):
    task = bz.main(name, list(map(lambda p: point(p.x * const.PIXELS_PER_MM, p.y * const.PIXELS_PER_MM), p_list)), settings)
    print_task(name, task)
